...
gnurrs coming out of the voodverk out is obvious, but what if we had a line that
was *MOSTLY* correct (like *"MOSTLY* dead," eh?). We'd parse the file to 
completion, but we'd be missing data. Necessary data? Essential data? Given
the above program, we'll never know.

This not knowing displeased Hilbert mightly.
